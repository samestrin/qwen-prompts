# /docs:standard - Documentation Standard Updater
...
### Format-only update
```qwen
/docs:standard Perform a 'format-only' update on @docs/old_api.md using the standard @docs/api_standard.md
```

### Content refresh with current code
```qwen
/docs:standard Do a 'content-refresh' on the user guide @docs/guide.md with the standard @docs/style_guide.md
```

### Complete rewrite based on current codebase
```qwen
/docs:standard Execute a 'full-rewrite' of @README.md using the template @docs/readme_standard.md
```

### Update multiple files to standard
```qwen
/docs:standard Apply 'content-refresh' to @docs/api.md @docs/cli.md using @docs/unified_standard.md
```
...
- [/docs:from-standard](from-standard.md) - Generate new documentation from standards
